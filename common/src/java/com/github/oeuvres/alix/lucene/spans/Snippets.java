 ...
 *
 * <p>
 * Raw Lucene spans are folded online. They are not stored. A new span continues
 * the current snippet when its start position is less than or equal to the
 * current snippet end plus the configured merge gap.
 * </p>
 *
 * <p>
 * Storage by usage:
 ...
 * <li>{@link Usage#OFFSETS}: stores snippet position ranges and matched-term
 * offsets.</li>
 * </ul>
 *
 * <p>
 * This class assumes that one token position corresponds to one visible token
 * offset. Under that analyzer/index invariant, matched leaves are deduplicated
 * by token position after sorting.
 * </p>
 *
 * <p>
 * Match slices belonging to each snippet are computed once in
 * {@link #closeDoc()}: every collected match falls into exactly one snippet
 * (matches only exist inside committed spans, and snippets do not overlap), so
 * a single forward sweep over sorted matches partitions them across snippets.
 * The slice is read at O(1) cost via {@link #snipStartMatchOrd(int)} and
 * {@link #snipEndMatchOrd(int)}, which use half-open bounds.
 * </p>
 *
 * <p>
...
public final class Snippets implements SpanCollector {
	...
	// OFFSETS
	/**
	 * Count of matches collected for current doc, after dedup once
	 * {@link #closeDoc()} has run
	 */
	...
	/**
	 * Half-open match-ordinal lower bound per snippet, of length
	 * {@code snips4doc + 1}. Entry at index {@code snips4doc} is the
	 * {@code matchCount} sentinel so that
	 * {@code snipEndMatchOrd(snipOrd) == snipMatchFrom[snipOrd + 1]} for every
	 * snippet without a special case.
	 */
	private int[] snipMatchFrom;

	/**
	 * Creates a snippet collector with default capacities.
	 *
	 * @param usage    collection level
	 * @param mergeGap maximum accepted gap, in token positions, between the current
	 *                 snippet end and the next span start. {@code 0} merges only
	 *                 touching or overlapping spans; positive values fuse spans
	 *                 separated by up to that many positions.
	 */
	public Snippets(final Usage usage, final int mergeGap) {
		...
		if (usage == Usage.OFFSETS) {
			...
			snipMatchFrom = new int[DEFAULT_SNIPPET_CAPACITY + 1];
		}
	}
	 ...
	 *
	 * <p>
	 * For {@link Usage#OFFSETS}, sorts matches by token position, deduplicates by
	 * position, then builds the per-snippet match slice ({@link #snipMatchFrom}).
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * In {@link Usage#OFFSETS} mode, appends one matched leaf occurrence. In other
	 * modes this method is a no-op.
	 * </p>
	 *
	 * <p>
	 * Order with respect to {@link #commitSpan(int, int)} is irrelevant: matches
	 * collected during one span go into a flat per-document pool that is sorted and
	 * partitioned across snippets in {@link #closeDoc()}. The driver may call
	 * {@code spans.collect(this)} either before or after {@code commitSpan}.
	 * </p>
	 *
	...
	/**
	 * Returns the count of documents that contributed at least one snippet since
	 * this collector was created. Accumulates across {@link #openDoc(int)} /
	 * {@link #closeDoc()} cycles.
	 *
	...
	/**
	 * Returns the count of distinct matches collected in the current document,
	 * after deduplication by token position.
	 *
	 * @return match count
	 * @throws IllegalStateException if called before {@link #closeDoc()} or when
	 *                               usage is not {@link Usage#OFFSETS}
	 */
	...
	/**
	 * Returns the character end offset of the {@code matchOrd}-th deduplicated
	 * match.
	 *
	 * @param matchOrd match ordinal
	 * @return character end offset
	 * @throws IllegalStateException     if called before {@link #closeDoc()} or
	 *                                   when usage is not {@link Usage#OFFSETS}
	...
	/**
	 * Returns the character start offset of the {@code matchOrd}-th deduplicated
	 * match.
	 *
	 * @param matchOrd match ordinal
	...
	/**
	 * Returns the exclusive upper bound of the match slice belonging to the
	 * {@code snipOrd}-th snippet. Iterate the snippet's matches with
	 * {@code for (int matchOrd = snipStartMatchOrd(snipOrd); matchOrd < snipEndMatchOrd(snipOrd);
	 *  matchOrd++)}.
	 *
	 ...
	 * @throws IllegalStateException     if called before {@link #closeDoc()} or
	 *                                   when usage is not {@link Usage#OFFSETS}
	 * @throws IndexOutOfBoundsException if {@code snipOrd} is outside
	 *                                   {@code [0, snips4doc())}
	 */
	public int snipEndMatchOrd(final int snipOrd) {
		requireFinished();
		requireOffsets();
		checkIndex(snipOrd, snips4doc, "snippet");
		return snipMatchFrom[snipOrd + 1];
	}

	/**
	 * Returns the end position (exclusive) of the {@code snipOrd}-th merged
	 * snippet.
	 *
	 * @param snipOrd snippet ordinal
	 * @return snippet end position
	 * @throws IllegalStateException     if called before {@link #closeDoc()} or
	 *                                   when usage is {@link Usage#FREQS}
	 * @throws IndexOutOfBoundsException if {@code snipOrd} is outside
	 *                                   {@code [0, snips4doc())}
	 */
	...
	/**
	 * Returns the inclusive lower bound of the match slice belonging to the
	 * {@code snipOrd}-th snippet.
	 *
	 * @param snipOrd snippet ordinal
	 ...
	 *                                   {@code [0, snips4doc())}
	 */
	public int snipStartMatchOrd(final int snipOrd) {
		requireFinished();
		requireOffsets();
		checkIndex(snipOrd, snips4doc, "snippet");
		return snipMatchFrom[snipOrd];
	}

	/**
	 * Returns the start position (inclusive) of the {@code snipOrd}-th merged
	 * snippet.
	 *
	 * @param snipOrd snippet ordinal
	...
	/**
	 * Builds {@link #snipMatchFrom} by walking sorted, deduplicated matches and the
	 * snippet array in parallel. Every match falls inside exactly one snippet, so
	 * the match cursor advances monotonically across snippets in a single pass. The
	 * final entry at index {@code snips4doc} is set to {@code matchCount} as a
	 * sentinel.
	 */
	private void buildSnipMatchFrom() {
		ensureSnipMatchFromCapacity(snips4doc + 1);

		int matchCursor = 0;
		for (int snipOrd = 0; snipOrd < snips4doc; snipOrd++) {
			snipMatchFrom[snipOrd] = matchCursor;
			final int snipEnd = unpackLow(snippets[snipOrd]);
			while (matchCursor < matchCount && unpackHigh(matches[matchCursor]) < snipEnd) {
				matchCursor++;
			}
		}
		snipMatchFrom[snips4doc] = matchCount;
	}
	...
	private static void checkIndex(final int index, final int count, final String label) {
		if (index < 0 || index >= count) {
			throw new IndexOutOfBoundsException(label + " index " + index + " outside [0, " + count + ")");
		}
	}
	...
	/**
	 * Ensures {@link #snipMatchFrom} can hold {@code required} entries (including
	 * the trailing sentinel).
	 */
	private void ensureSnipMatchFromCapacity(final int required) {
		if (snipMatchFrom.length >= required) {
			return;
		}

		int length = snipMatchFrom.length << 1;
		while (length < required) {
			length <<= 1;
		}
		snipMatchFrom = Arrays.copyOf(snipMatchFrom, length);
	}
	...
	/**
	 * Sorts matches by token position and removes duplicate positions in place. The
	 * relative order of {@code matches[i].low} (the index into
	 * {@code matchOffsets}) is preserved by the long-key sort: each surviving entry
	 * still points to its own offset pair. When two matches share a position, the
	 * survivor is the one with the smaller {@code matchOffsetsIndex}, i.e. the
	 * earliest-arrived occurrence.
	 */
