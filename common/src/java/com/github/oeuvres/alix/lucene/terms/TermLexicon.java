...
import com.github.oeuvres.alix.lucene.terms.TermTagger.TagSink;
 ...
 *       never spells, suggests or corrects on its own. The {@link Dictionary} is immutable and
 *       concurrent-safe; the non-thread-safe {@code Hunspell} runtime is the caller's to create per
 *       thread.</li>
 *   <li>term-tag membership: one {@link BitSet} of term ids per {@link TermTag} actually set. Part-of-speech
 *       tags (ADJ/NOUN/VERB/X) are harvested inline from the {@code dic}'s {@code po:} fields during the
 *       same scan that builds the dictionary; all other tags are produced by {@link TermTagger}s passed to
 *       the constructor and run once after the table is built. A term id may carry several tags. Query with
 *       {@link #tags(int)}, {@link #bits(TermTag)} or {@link #has(int, TermTag)}.</li>
 * </ul>
 * <p>
 ...
 * heap state, and taggers run synchronously within the constructor, so the instance is frozen once it
 * escapes. The join between any external form (dictionary headword, authority-list entry) and an indexed
 * term is a raw byte match through {@link #id(String)}; supply forms in the same normalized shape the
 * analyzer produced or coverage silently drops.
 * </p>
 *
 * @see TermRail
 * @see TermTag
...
public final class TermLexicon {
    ...
    /**
     * Term-tag membership: for each {@link TermTag} actually set, the term ids carrying it. Only tags that
     * were set ever get a BitSet; never {@code null}, empty when nothing was tagged.
     */
    private final EnumMap<TermTag, BitSet> tagBits;

    /**
     * Builds the bare lexicon for one field: no Hunspell dictionary, no tags.
     *
     ...
     * inline part-of-speech tags, and tagger-produced tags. The reader and both streams are consulted only
     * here and are not retained; the streams are read once and are not closed (the caller keeps
     * ownership). Taggers run after the table is built, in the order given.
     * <p>
     * Effect of the nullable Hunspell sources:
     * </p>
     * <ul>
     *   <li>{@code dic == null}: no dictionary, no part-of-speech tags; an {@code aff} given alone is
     *       ignored. Taggers still run.</li>
     *   <li>{@code dic != null}, {@code aff == null}: part-of-speech tags are harvested from {@code po:};
     *       {@link #hunspell()} is {@code null}, since a {@link Dictionary} needs the affix rules.</li>
     *   <li>{@code dic != null}, {@code aff != null}: part-of-speech tags are harvested and
     *       {@link #hunspell()} returns the restricted dictionary (or {@code null} if no headword is indexed
     *       in this field).</li>
     * </ul>
     *
     * @param reader  snapshot reader that defines the term universe and its lexicographic order
     * @param field   indexed field name
     * @param dic     canonical Hunspell {@code .dic} (count header on line one, optional {@code po:}
     *                fields), or {@code null}
     * @param aff     Hunspell {@code .aff} matching {@code dic}, or {@code null}
     * @param taggers term-tag fillers, run in order after the table is built
     * @throws IOException              if reading the term dictionary, {@code dic}, or a tagger resource
     *                                  fails, if 32-bit limits are exceeded, or if the Hunspell dictionary
     *                                  cannot be parsed
     * @throws IllegalArgumentException if the field has no terms in the reader
     * @throws NullPointerException     if {@code reader} or {@code field} is null
     */
    public TermLexicon(final IndexReader reader, final String field,
            final InputStream dic, final InputStream aff, final TermTagger... taggers) throws IOException {
        ...
        final EnumMap<TermTag, BitSet> bits = new EnumMap<>(TermTag.class);
        final TagSink sink = (tag, id) -> bits.computeIfAbsent(tag, k -> new BitSet(vocabSize)).set(id);
        ...
        if (taggers != null) {
            for (final TermTagger tagger : taggers) {
                if (tagger != null) {
                    tagger.tag(this, sink);
                }
            }
        }
    }
    ...
    /**
     * Returns the term ids carrying one tag.
     * <p>
     * The result is a defensive copy: the caller may read, iterate or mutate it without affecting the
     * lexicon. An empty BitSet is returned when the tag was never set. For pivot queries, intersect this
     * with a co-occurrence candidate set (e.g. {@code bits(TermTag.PERS)} AND the cooc terms of a pivot).
     * </p>
     *
     * @param tag membership category
     * @return a fresh {@link BitSet} of term ids, never null
     * @throws NullPointerException if {@code tag} is null
     */
    public BitSet bits(final TermTag tag) {
        Objects.requireNonNull(tag, "tag");
        final BitSet bs = tagBits.get(tag);
    ...
    /**
     * Tells whether one term id carries one tag, without allocating.
     *
     * @param termId dense term id in {@code [0, vocabSize)}
     * @param tag    membership category
     * @return true iff the term is tagged
     * @throws IllegalArgumentException if {@code termId} is out of range
     * @throws NullPointerException     if {@code tag} is null
     */
    public boolean has(final int termId, final TermTag tag) {
        checkTermId(termId);
        Objects.requireNonNull(tag, "tag");
        final BitSet bs = tagBits.get(tag);
    ...
    /**
     * Returns the tags carried by one term id.
     * <p>
     * Empty when the term was never tagged. A term may carry several tags.
     * </p>
     *
     * @param termId dense term id in {@code [0, vocabSize)}
     * @return a fresh, mutable {@link EnumSet} of tags; empty if none
     * @throws IllegalArgumentException if {@code termId} is out of range
     */
    public EnumSet<TermTag> tags(final int termId) {
        checkTermId(termId);
        final EnumSet<TermTag> set = EnumSet.noneOf(TermTag.class);
        for (final Map.Entry<TermTag, BitSet> e : tagBits.entrySet()) {
    ...
    /**
     * Sets part-of-speech bits for one kept dictionary line, reading every {@code po:} field on it. A
     * value is used only when it names a {@link TermTag} constant, so the enum decides what is harvested.
     *
     * @param line   raw dictionary line
     * @param termId resolved term id for the line's headword
     * @param sink   destination for tag membership
     */
    private void harvestPos(final String line, final int termId, final TagSink sink) {
        ...
        while (i >= 0) {
            ...
            final TermTag tag = termTagOf(line.substring(start, end));
            if (tag != null) {
                sink.set(tag, termId);
            }
    ...
    /**
     * Maps a {@code po:} value to a {@link TermTag}, returning {@code null} for any value that is not a
     * constant name, so unmodelled parts of speech are skipped.
     *
     * @param po the value after {@code po:}
     * @return matching tag, or {@code null}
     */
    private static TermTag termTagOf(final String po) {
        try {
            return TermTag.valueOf(po);
