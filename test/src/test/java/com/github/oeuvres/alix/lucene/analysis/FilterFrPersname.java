...
public class FilterFrPersname extends TokenFilter
{
    ...
    public final boolean incrementToken() throws IOException
    {
        ...
        if (flags == NAMEpersf.code || flags == NAMEpersm.code) {
            tagEnd = NAMEpers.code;
            nameFound = true;
        }
        // found as a name (capitalization resolved)
        else if (NAME.isName(flags) && Char.isUpperCase(termAtt.charAt(0))) {
            tagEnd = flags;
            nameFound = true;
        }
        // possible candidates
        else if (flags == SUBpers.code) { // Madame, Maître…
            tagEnd = NAMEpers.code;
            stack.add(captureState()); // store state in case of rewind (Monsieur Madeleine YES, Madame va bien ? NO)
        } else if (flags == SUBplace.code) { // Rue, faubourg…
            tagEnd = NAMEplace.code;
        ...
        if (flagsAtt.getFlags() == SUBpers.code)
        ...
        while ((exit = input.incrementToken())) {
            ...
            if (Char.isUpperCase(termAtt.charAt(0))) { // TagFr.NAME.sameParent(flags2)
                ...
                if (flags2 == NAMEplace.code && tagEnd != NAMEpers.code)
                    tagEnd = NAMEplace.code; // Le [comte de Toulouse] is a person, not a place
                else if (flags2 == NAMEpers.code)
                    tagEnd = NAMEpers.code;
                else if (flags2 == NAMEpersf.code)
                    tagEnd = NAMEpers.code;
                else if (flags2 == NAMEpersm.code)
                    tagEnd = NAMEpers.code;
                // separator
                String sep = " ";
                ...
                if (concLem.isEmpty() && (flags2 == NAMEpersm.code || flags2 == NAMEpersf.code))
                    ; // de Suzon
                else {
                    if (concLem.isEmpty())
        ...
        // no name found in the go ahead, replay stack
