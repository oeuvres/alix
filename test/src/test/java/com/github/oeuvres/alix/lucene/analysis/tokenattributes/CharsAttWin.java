...
{
    private final CharsAttImpl[] data;
    /** Size of left wing */
    ...
    public CharsAttWin(final int left, final int right) {
        if (left > 0)
            throw new IndexOutOfBoundsException(
                    this.getClass().getName() + " left context should be negative or zero.");
        this.left = left;
        if (right < 0)
            throw new IndexOutOfBoundsException(
                    this.getClass().getName() + " right context should be positive or zero.");
        this.right = right;
        size = -left + right + 1;
        center = left;
        data = new CharsAttImpl[size];
        for (int i = 0; i < size; i++)
            data[i] = new CharsAttImpl();
    }
     ...
     */
    public CharsAttImpl get(final int pos)
    {
     ...
     */
    public CharsAttWin push(final CharsAttImpl term)
    {
        center = pointer(+1);
