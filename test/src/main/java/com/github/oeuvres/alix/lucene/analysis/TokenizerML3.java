 ...
 */
public class TokenizerML3 extends Tokenizer
{
    ...
    public final boolean incrementToken() throws IOException
    {
        ...
        while (true) {
            ...
            if (abbrevDot) {
                if (!isLetterFast(c)) {
                    // Keep one-letter abbreviation "M." (len==2) as a single token (original behavior).
                    if (!(termLen == 2 && isLetterFast(termBuf[0]))) {
                        // Detach '.' and push it back as punctuation.
                        termLen--;
                        pendingChar = '.';
                        pendingCharOffset = off - 1; // '.' already consumed previously
                        tokenEndOff = off - 1;       // word ends BEFORE dot
                        abbrevDot = false;
                        break; // emit word without '.'
                    }
                    // else keep dot; fall through (and likely break on delimiter)
                }
            ...
            if (c == '.' && termLen > 0 && isLetterFast(lastChar)) {
                if (termLen == termBuf.length) termBuf = termAtt.resizeBuffer(termLen + 1);
                termBuf[termLen++] = '.';
                bi++; off++; lastChar = '.';
                abbrevDot = true;
    ...
    private static boolean isLetterFast(char c) {
        // ASCII letter fast-path + fallback
        final int x = c | 32;
        return (x >= 'a' && x <= 'z') || Char.isLetter(c);
    }

    private static boolean isTokenFast(char c) {
        // fast-path for ASCII letters; everything else delegate to Char.isToken
        final int x = c | 32;
        return (x >= 'a' && x <= 'z') || Char.isToken(c);
    }
}
