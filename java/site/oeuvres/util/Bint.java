...
/**
 * 
 * An int-int map using an internal binary tree.
 * The Vek hash implementation is faster, and use less memory.
 * 
 * To optimize write, a local buffer is growing with no sort, no uniq, on insert().
 * A commit() is required before get().
 * No remove() implemented yet, good strategy maybe to set an "empty value", till a vacuum() is run.
 * 
 * 
 * The good idea is to associate int key and int value in a long entry.
 * Data is a long array.
 * Key is the high position of the long entry so that sort is in key order.
 * 
 */
public class Bint
{
  ...
  /**
   * Most of the logic, a kind of uniq on keys
   * with a merge strategy for duplicate keys
   */
  public void uniq() {
   ...
   * Insert an entry
   */
  public Bint insert( final int key, final int value ) {
   ...
   * All pended insert should have been commited.
   * @param key
   * @return
   */
  public boolean contains(final int key)
  {
    ...
    if ( key( data[pos] ) == key ) return true;
    return false;
  }
  /**
   * Get a value by key
  ...
  public int get( final int key )
  {
    ...
    if ( pos < 0 ) pos = -pos-1;
    if ( key( data[pos] ) != key ) return NO_VALUE;
  ...
  {
    return ( ((long)key) << 32 ) | (((long)value) & KEY_MASK);
  }
  ...
  {
    if (!(obj instanceof Bint))
      return false;
    if (obj == this)
      return true;
    int[][] a = ((Bint)obj).toArray();
    int[][] b = ((Bint)obj).toArray();
  ...
  public static void main(String[] args)
  {
    ...
    System.out.print( "Vek "+4*max+" inserts"  );
    Vek vek = new Vek(max);
    ...
    System.out.print( "Bint "+4*max+" inserts"  );
    Bint bint = new Bint(max, ADD);
    for (int i=0 ; i < 4*max; i++) {
