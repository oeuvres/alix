 ...
 * TODO : send an event to get some occurrence more ?
 * @author user
 *
 */
public class Lexer
{
  ...
  public void load( BufferedReader buf ) throws IOException
  {
    ...
    while ( ( line = buf.readLine() ) != null ) {
      ...
      if ( center instanceof TestTerm ) {
        ...
        if ( nos == null ) byform.put( term, new int[]{ no } );
        else {
        ...
        int key = 0;
        if ( center instanceof TestTag ) key = Tag.prefix( ((TestTag)center).tag );
        else if ( center instanceof TestTagPrefix ) key =  ((TestTagPrefix)center).prefix ;
        int[] nos = bytag.get( key );
        if ( nos == null ) bytag.put( key, new int[]{ no } );
        else {
          int oldlength = nos.length;
          nos = Arrays.copyOf( nos, oldlength+1 );
          nos[ oldlength ] = no;
  ...
  public boolean apply( Occ occ )
  {
    ...
    if ( rules1 == null && rules2 == null ) return false;
    else if ( rules1 != null ) rules = rules1;
    else if ( rules2 != null ) rules = rules2;
    ...
    for( int no:rules ) {
      ...
      if ( rulebook[no].apply( occ ) ) return true;
    }
    return false;
  }
  ...
  static public Test test( String term )
  {
    ...
    if ( pos < 0 ) {
      ...
      if ( tag == Tag.UNKNOWN ) return new TestOrth( term.substring( start, end ) );
      if ( Tag.isPrefix( tag ) && !quotes ) return new TestTagPrefix( tag );
      return new TestTag( tag );
    }
    else {
      tag = Tag.code( term.substring( pos+1, end ) );
      if ( Tag.isPrefix( tag ) && !quotes ) return new TestOrthTagPrefix( term.substring( start, pos ), tag );
  ...
  private class Rule
  {
    ...
    /**
     * Test if rule is relevant to an occurrence and its context
     * TODO: how to get an occurrence next if needed to test rule?
     * @param center
    ...
    public boolean match( final Occ occ )
    {
      ...
      if ( !center.test( occ ) ) return false;
      // test left context
      Occ context = occ;
      Test test = center;
      while ( test.prev() != null ) {
        test = test.prev();
        if ( context.prev() == null ) return false;
        context = context.prev();
        if ( !test.test( context ) ) return false;
      }
      // test right context
      context = occ;
      test = center;
      while ( test.next() != null ) {
        test = test.next();
        if ( context.next() == null ) return false;
  ...
  public String toString()
  {
    ...
    for ( int i=0; i < length; i++ ) {
      ...
      while ( test.prev() != null ) test = test.prev();
      // print the chain from start
      first = true;
      while ( test != null ) {
        if ( test == rule.center ) sb.append( sep );
