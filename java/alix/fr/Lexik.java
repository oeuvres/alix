...
public class Lexik
{
  ...
  public static void load(BufferedReader buf, final int mode, final String src) throws IOException, ParseException
  {
    ...
    while ((l = buf.readLine()) != null) {
      ...
      // else if (mode == _RULES) action = _RULES; // do not register locutions from
      // stop words
      // be careful on apos here for d' or l' in dico
      // else if (cells[0].indexOf(' ') > 0 || cells[0].indexOf('\'') >= 0) action =
      // _LOC;
      ...
      /*
       * else if (action == _RULES) { RULES.add(cells); continue; } else if (action ==
       * _LOC) { LOC.add(cells); continue; }
       */
  ...
  {
    if (!ORTH.containsKey(chain)) return false;
  ...
  /**
   * Compare dictionaries, names should not contain common words
   * 
   * @throws ParseException
   * @throws IOException
   */
