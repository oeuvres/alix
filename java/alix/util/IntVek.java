 ...
 * -2147483648 as a key (Integer.MIN_VALUE), is used to tag empty value, no warning will be sent.
 * 
 * 
 * source:
...
public class IntVek implements Cloneable
{
    ...
    public static void main(String[] args)
    {
        ...
        for (int i = 0; i < max; i++) {
          for (int j = 0; j < max; j++) {
            System.out.println(
                "" + i + "–" + j + "   " + list.get(i).intercos(list.get(j)) + " " + list.get(i).cosine(list.get(j)));
            System.out.println(list.get(i).specs(list.get(j)));
          }
        }
        */
        /* test if storing is OK
        IntVek vec = new IntVek();
        for (int i=-50000; i < 50000; i++) vec.put(i, i);
        for (int i=-50001; i < 50001; i++) {
            int get = vec.get(i);
            if (i != get) System.out.println(i+" != "+get);;
        }
        while(vec.next()) {
            vec.set(vec.value()+1);
        }
        for (int i=-50001; i < 50001; i++) {
            int get = vec.get(i);
            if ( (i+1) != get) System.out.println( (i+1)+" != "+get);;
        }
        */
    }
