...
public class SparseMat
{
    ...
    public void ppmi(double laplace)
    {
        ...
        for (int i = 0; i < length; i++) {
            double cell = Math
                    .log((double) ((counts[i] + laplace) * sum) / (double) (rowSum[rows[i]] * colSum[cols[i]])) / LOG2;
            if (cell < 0)
                cell = 0;
            else
                rowMag[rows[i]] += cell * cell;
            ppmi[i] = cell;
        }
        for (int i = 0; i < height; i++)
    ...
    public Top<Integer> sims(int row, final int mode, int topSize)
    {
        ...
        double dist;
        for (int i = 0; i < height; i++) {
            if (mode == SparseMat.PPMI)
                dist = cosine(row, i, ppmi, ppmiRowMag);
            else
                dist = cosine(row, i, counts, countsRowMag);
            top.push(dist, i);
        }
        return top;
    }
