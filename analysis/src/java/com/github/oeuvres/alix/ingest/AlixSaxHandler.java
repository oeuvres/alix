...
public final class AlixSaxHandler extends DefaultHandler2
{
    ...
    /**
     * The <br/>
     * bug, pending start tag text, or null if no tag is waiting to be closed.
     */
    private String pendingTag = null;
    
    /** To control escaping in CDATA */
     ...
     *
     * <p>
     * Non-alix elements inside a {@link FieldMode#CONTENT} field are serialized
     * as escaped XML markup into the field buffer. Non-alix elements outside a
     * content field are rejected.
     * </p>
     */
    @Override
     ...
     *
     * <p>
     * Non-alix end elements inside a content field are serialized as closing tags.
     * Non-alix end elements at payload depth zero are silently ignored (the matching
     * start element would already have been rejected if invalid).
     * </p>
     */
    @Override
    public void endElement(String uri, String localName, String qName) throws SAXException
    {
        ...
        switch (localName) {
            ...
            case "set" -> endSet();
            default -> {
                /* ignore unknown alix end tags (startElement already validated) */ }
        }
    }
     ...
     *
     * <p>
     * In {@link FieldMode#CONTENT}, characters are XML-escaped and appended to the
     * field buffer. In {@link FieldMode#SCALAR} or {@link FieldMode#DERIVED}, only
     * whitespace is tolerated (inline content is forbidden).
     * </p>
     *
    ...
    public void characters(char[] ch, int start, int length) throws SAXException
    {
        ...
        switch (fieldMode) {
            ...
            case CONTENT -> {
                if (inCDATA) doc.fieldChars(ch, start, length);
                else appendEscapedText(ch, start, length);
            }
        }
    }
     ...
     *
     * <p>
     * Validating parsers may deliver inter-element whitespace here instead of
     * through {@code characters()}. The handling is identical: whitespace is
     * tolerated in all field modes, content is rejected in scalar/derived fields.
     * </p>
     */
    @Override
    ...
    private void appendStartTag(String name, Attributes atts)
    {
        ...
        pendingTag = tagBuf.toString(); // store without closing >
    }
    
    private void flushPendingTag()
    {
        if (pendingTag != null) {
            doc.fieldText(pendingTag + ">");
            pendingTag = null;
        }
    }
    ...
    {
        flushPendingTag(); // content follows: close the start tag normally
        int end = start + len;
        int last = start;
    ...
    {
        if (pendingTag != null) {
            doc.fieldText(pendingTag + " />"); // self-closing
            pendingTag = null;
        } else {
            tagBuf.setLength(0);
     ...
     *
     * <p>
     * A book is first accumulated as a logical document ({@link OpenDoc#BOOK}).
     ...
     * or at {@code </alix:book>} if there are no chapters.
     * </p>
     *
     ...
     *
     * <p>
     * If the book-level document has not yet been emitted, it is emitted now
     * (all book-level fields accumulated so far). Chapters get synthetic
     * {@code ALIX_FILENAME}, {@code ALIX_BOOKID}, and {@code ALIX_ORD} fields.
     * </p>
     *
     * <p>
     * If no {@code xml:id} is present, a default id is generated as
     * {@code bookId + "-" + ordinal} (zero-padded to at least two digits).
     * </p>
     *
     ...
     *
     * <p>
     * Tries, in order: {@code getValue("", localName)}, {@code getValue(localName)},
     ...
     *
     * <p>
     * Tries: {@code (XML_NS, "id")}, then {@code "xml:id"}, then plain {@code "id"}.
     * </p>
     *
